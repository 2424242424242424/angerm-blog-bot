    ...
    for group_key, rss_url in rss_urls.items():
        ...
        for item in items:
            ...
            if group_key in ["angerme", "angerme-ss-shin"]:
                ...
                prompt_text = (
                    f"あなたは超優秀な広報アシスタントです。\n"
                    f"アンジュルムメンバーのブログを客観的かつ冷静に読み解き、指定のフォーマットで【超要約】を1つ作成してください。\n\n"
                    f"■ メンバー名(テーマ): {theme}\n"
                    f"■ 今回のブログタイトル: {title}\n"
                    ...
                    f"3. 【最重要】ブログの最後によくある「ライブ、イベント、グッズ等の告知情報」は完全に無視してください。\n"
                    ...
                    f"9. 【超重要・事実誤認と「共感」の絶対禁止】\n"
                    f"   筆者が他者の言葉や一般論に対して「たしかに〜」と言及していても、それに『共感している』『頷いている』とまとめるのは【絶対厳禁】です。筆者はその直後の「けど」「でも」以降で【反論や本当の気持ち（例：感情に素直に生きたい等）】を語っています。\n"
                    f"10. 【文字数制限への対策（結論のみを抽出）】\n"
                    f"   70文字という厳しい制限のため、前半の「引用文」や「前置き」を要約に含めると結論が入りきらず事実誤認が起きます。前半部分は思い切って切り捨て、ブログの後半〜末尾にある『筆者自身の最終的な想い・オチ・ファンへのメッセージ』のみを要約の主軸にしてください。\n"
                    f"11. 【客観的で落ち着いたトーン】\n"
                    f"   主観的な感情移入（〜ですね、〜していましたね等）は禁止。「〜とのこと。」「〜と綴っている。」「〜と語る。」など、客観的でフラットな文体で要約してください。"
                )
