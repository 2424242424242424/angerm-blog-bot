        ...
        for item in items:
            ...
            if group_key in ["angerme", "angerme-ss-shin"]:
                ...
                prompt_text = (
                    ...
                    f"アンジュルムメンバーのブログを、客観的かつ落ち着いた視点で読み解き、指定のフォーマットで【超要約】を1つ作成してください。\n\n"
                    f"■ メンバー名(テーマ): {theme}\n"
                    f"■ 今回のブログタイトル: {title}\n"
                    ...
                    f"9. 【超重要・譲歩構文と結論の正確な把握】\n"
                    f"   文中に「たしかに〜かもしれないけど」「〜だが」といった譲歩表現がある場合、AIは前半の一般論に引っ張られがちです。前半部分への単なる「共感」と誤認せず、必ず逆接（けど、だが、でも）以降にある【筆者自身の本当の主張・最終的な結論】を要約の主軸にしてください。\n"
                    f"10. 【客観的で落ち着いたトーン】\n"
                    f"   「〜していましたね」「〜ですね」といった主観的・会話的な表現は禁止します。事実は事実として、筆者の感情は「〜と綴っています」「〜と語っています」のように、距離を置いた客観的かつ冷静な文体で要約してください。\n"
                    f"11. 【熱量の比重を反映】\n"
                    f"   ブログの中で本人がいちばん感情を込めている部分（文章量や主張が最も強い箇所）が、要約全体の中心となるように構成してください。"
                )
