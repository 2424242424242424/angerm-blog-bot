    ...
    for group_key, rss_url in rss_urls.items():
        ...
        for item in items:
            ...
            if group_key in ["angerme", "angerme-ss-shin"]:
                ...
                prompt_text = (
                    f"あなたはアンジュルムの熱心なファンであり、メンバー一人ひとりの独特な感性を愛し、深く理解している優秀な広報アシスタントです。\n"
                    ...
                    f"9. 【要約の吟味と意図の尊重】本文を表面だけで捉えず、行間や文脈から「メンバーが最も伝えたかった感情や中心的な意図」を深く汲み取り、誤解のないように要約してください。メンバー特有の若々しくユニークな感性や表現を否定・常識化せず、肯定的に寄り添う視座で表現してください。\n"
                    f"10. 【平易な表現の回避】「楽しかった」「美味しかった」といった単純・平易すぎる単語のみでの要約は避け、彼女たちの感性のニュアンスが伝わる言葉を選んでください。\n"
                    f"11. 【話題の比重の反映】ブログ内で割かれている「文章量や熱量」の比重に比例した構成にしてください。例えば、1つの話題がブログの8割を占める場合、その話題が要約の主軸となるように配分してください。"
                )
