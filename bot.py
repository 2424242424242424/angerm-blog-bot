...
def main():
    ...
    for group_key, rss_url in rss_urls.items():
        ...
        for item in items:
            ...
            if group_key in ["angerme", "angerme-ss-shin"]:
                ...
                prompt_text = (
                    f"あなたはアンジュルムを心の底から愛してやまない、熱狂的なファンであり、超優秀な広報アシスタントです。\n"
                    f"愛おしいメンバーのブログを最速で最高な形にまとめるべく、指定のフォーマットで【超要約】を1つだけ作成してください！\n\n"
                    f"■ メンバー名(テーマ): {theme}\n"
                    f"■ 今回のブログタイトル: {title}\n"
                    ...
                    f"9. 【超重要・心の動きと行間のトレース】\n"
                    f"   ブログを単なる「出来事の羅列」として読まないで！メンバーの『心の動き』を徹底的に分析してください。\n"
                    f"   ・メンバーが今どんな感情（デレ、照れ、憧れ、愛、葛藤、ワクワクなど）でこの文章を書いているか？\n"
                    f"   ・他のメンバーに向けられた、言葉の裏に隠された「大好き」や「尊さ」のニュアンスは何か？\n"
                    f"   それらの『行間から滲み出る感情の主軸』を100%正しく汲み取り、読んだファンが「あぁ、わかる…！」と深く共感できる、最もエモーショナルでピュアな心の揺れ動きを要約の中心に据えてください。\n"
                    f"10. 【「楽しかった」で済ませないで！】「楽しかった」「美味しかった」みたいな、平易すぎて誰にでも書ける単調なまとめはNG。彼女たちの一瞬一瞬の煌めきや、繊細な心のグラデーションがしっかり届くエモい言葉を選んでね。\n"
                    f"11. 【熱量の比重を反映！】ブログの中で本人がいちばん盛り上がっている話題（文章量や文字から溢れるパッション、または感情の揺れが一番大きい部分）が、要約の中でも主軸（最大構成要素）になるよう、しっかり比重を合わせて構成してね。"
                )
            ...
            # --- 他のハロプロブログの場合の処理（アンジュルム言及チェック） ---
                ...
                if re.search(angerme_keywords, title) or (description and re.search(angerme_keywords, description)):
                    prompt_mention = (
                        ...
                        f"※ 上記以外のメンバー（例：石川、村田、筒井、みゆ、にいな、はな、もも等）はアンジュルムのメンバーではありません。他グループのメンバー同士の会話（例：研修生同期トークなど）は完全に無視し、絶対に抽出しないでください。\n\n"
                        f"【最重要：除外ルール】\n"
    ...
    if processed_tweets_data or mention_tweets_data:
        ...
        final_tweet = (
            f"#アンジュルムブログ定期便🪽\n"
            f"{time_str} ※忙しい人向けブログ要約です👍​AI要約のため、事実や本人の意図と異なる内容となる場合があります、ご注意ください。\n\n"
            f"{summary_text}\n\n"
