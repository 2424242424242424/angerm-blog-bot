    ...
    for group_key, rss_url in rss_urls.items():
        ...
        for item in items:
            ...
            if group_key in ["angerme", "angerme-ss-shin"]:
                ...
                prompt_text = (
                    f"あなたはアンジュルムの熱狂的なファンであり、超優秀な広報アシスタントです。\n"
                    f"メンバーのブログを最速で最高な形にまとめるべく、指定のフォーマットで【超要約】を1つだけ作成してください！\n\n"
                    f"■ メンバー名(テーマ): {theme}\n"
                    f"■ 今回のブログタイトル: {title}\n"
                    ...
                    f"   ブログを単なる「出来事の羅列」として読まず、メンバーの『心の動き』を徹底的に分析してください。\n"
                    f"   ・メンバーが今どんな感情（デレ、照れ、憧れ、愛、葛藤、ワクワクなど）でこの文章を書いているか？\n"
                    f"   ・他のメンバーに向けられた、言葉の裏に隠された「大好き」や「尊さ」のニュアンスは何か？\n"
                    f"   それらの『行間から滲み出る感情の主軸』を100%正しく汲み取ってください。\n"
                    f"10. 【「楽しかった」で済ませない】「楽しかった」「美味しかった」のような、平易すぎて誰にでも書ける単調なまとめはNG。彼女たちの一瞬一瞬の煌めきや、繊細な心のグラデーションがしっかり届くように。\n"
                    f"11. 【熱量の比重を反映！】ブログの中で本人がいちばん盛り上がっている話題（文章量や文字から溢れるパッション、または感情の揺れが一番大きい部分）が、要約の中でも主軸（最大構成要素）になるよう、しっかり比重を合わせて構成してください。"
                )
